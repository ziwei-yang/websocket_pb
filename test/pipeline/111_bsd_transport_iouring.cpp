// test/pipeline/111_bsd_transport_iouring.cpp
// Test BSDSocketTransportProcess_IoUring (Option E) against TCP echo server
//
// Usage:
//   ./test_pipeline_bsd_transport_iouring                    # localhost echo (default)
//   ./test_pipeline_bsd_transport_iouring localhost          # localhost echo server
//   ./test_pipeline_bsd_transport_iouring <ip> <port>        # remote echo server
//
// This test:
// - Creates IPC rings (MSG_OUTBOX, MSG_METADATA, PONGS)
// - Optionally starts a localhost TCP echo server
// - Forks BSDSocketTransportProcess_IoUring (single-thread io_uring)
...
// - liburing library
//
// Unlike Option B (2-thread):
// - Single-threaded async I/O model
// - Uses io_uring for non-blocking recv/send
// - Potentially lower latency (no cross-thread communication)

#ifndef __linux__
#error "This test requires Linux with io_uring support"
#endif

// pipeline_data.hpp must be included FIRST to avoid macro conflicts
...
constexpr uint16_t LOCALHOST_ECHO_PORT = 18345;            // Port for localhost echo server
...
public:
    IPCRingManager() {
        ...
        ipc_ring_dir_ = std::string("bsd_iouring_test_") + timestamp;
    }
...
using IoUringTransportType = BSDSocketTransportProcess<
...
public:
    IoUringTransportTest(const char* echo_ip, uint16_t echo_port, bool use_localhost)
        : echo_ip_(echo_ip), echo_port_(echo_port), use_localhost_(use_localhost) {}

    bool setup() {
        printf("\n=== Setting up io_uring Transport Test ===\n");
    ...
    bool fork_transport_process() {
        ...
        printf("[PARENT] Forked io_uring Transport process (PID %d)\n", transport_pid_);
        ...
        printf("[PARENT] Connection established (io_uring mode)\n");

        return true;
    }

    bool run_roundtrip_test() {
        printf("\n--- io_uring Roundtrip Echo Test (max %d msgs, timeout %ds, interval %dms) ---\n",
        ...
        auto process_metadata = [&](const MsgMetadata& meta) {
            ...
            while (pos + MSG_PREFIX_LEN + 9 <= meta.decrypted_len) {
                ...
                if (data[pos + TSC_HEX_LEN] == ':' &&
                    ...
                    received++;
                    pos += MSG_PREFIX_LEN + 13;  // Skip past this message
                } else {
                    pos++;
                }
            }
        ...
        while (sent < MAX_MESSAGES) {
            ...
            if (since_last >= SEND_INTERVAL_MS) {
                ...
                // Prepare test message with embedded TSC
                char msg[64];
        ...
        rtt_latency.print("Full Round-Trip Latency (io_uring async I/O)");
...
private:
    ...
    void run_transport_process() {
        ...
        IoUringTransportType transport;
...
int main(int argc, char* argv[]) {
    ...
    printf("==============================================\n");
    printf("  io_uring Transport Test (Option E)          \n");
    printf("==============================================\n");
    printf("  Target:    %s:%u%s\n", echo_ip, echo_port,
           use_localhost ? " (localhost echo)" : "");
    printf("  Messages:  up to %d (every %dms)\n", MAX_MESSAGES, SEND_INTERVAL_MS);
    printf("  Timeout:   %d seconds\n", TIMEOUT_SECONDS);
    printf("  Mode:      Single-thread io_uring async I/O\n");
    printf("==============================================\n\n");

    IoUringTransportTest test(echo_ip, echo_port, use_localhost);
