// test/pipeline/13_transport_https_openssl.cpp
// Test TransportProcess<OpenSSLPolicy> with XDP Poll against HTTPS server
//
// Usage: ./test_pipeline_transport_https_openssl <interface> <bpf_path> <ignored> <ignored>
// (Called by scripts/test_xdp.sh 13_transport_https_openssl.cpp)
//
// Build: make build-test-pipeline-transport_https_openssl USE_XDP=1
//
// This test:
// - Forks XDP Poll process (core 2) and Transport process (core 4)
// - Connects to nginx.org on port 443 (HTTPS with OpenSSL)
...
#include "../../src/policy/ssl.hpp"  // OpenSSLPolicy
...
public:
    IPCRingManager() {
        ...
        ipc_ring_dir_ = std::string("transport_https_openssl_test_") + timestamp;
    }
...
class TransportHTTPSOpenSSLTest {
public:
    TransportHTTPSOpenSSLTest(const char* interface, const char* bpf_path)
        : interface_(interface), bpf_path_(bpf_path) {}

    bool setup() {
        printf("\n=== Setting up Transport HTTPS OpenSSL Test ===\n");
        printf("Interface:   %s\n", interface_);
        printf("BPF Path:    %s\n", bpf_path_);
    ...
    bool fork_processes() {
        ...
        printf("[PARENT] Waiting for TLS handshake (OpenSSL)...\n");
        ...
        printf("[PARENT] TLS handshake complete (OpenSSL)\n");
    ...
    bool run_https_test() {
        ...
        auto process_response_chunk = [&](const MsgMetadata& meta) {
            ...
            if (is_response_start) {
                ...
                if (conn_header && (conn_header - data) < static_cast<ssize_t>(meta.decrypted_len)) {
                    fprintf(stderr, "[WARN] Server sent 'Connection: close'\n");
                }
            } else {
                ...
                if (in_response && chunk_start != last_chunk_end) {
                    if (chunk_start < current_response_start) {
                        fprintf(stderr, "[ORDER] ERROR: Chunk offset %u < response start %u\n",
            ...
            if (accumulated_response.find("</html>") != std::string::npos ||
                ...
                printf("[RECV] Complete response #%d received (%zu bytes)\n", complete_responses, resp_size);
                in_response = false;
        ...
        while (sent < MAX_MESSAGES) {
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(now - last_debug).count() >= 1) {
                ...
                printf("[DEBUG] MSG_OUTBOX prod=%ld cons=%ld | sent=%d\n", msg_out_prod, msg_out_cons, sent);
                last_debug = now;
            }
        ...
        if (!connection_closed && !conn_state_->is_running(PROC_TRANSPORT)) {
            fprintf(stderr, "[TEST] Transport died after sending\n");
            keep_alive_working = false;
        }
        ...
        while (true) {
            ...
            if ((min_drain_done && all_responses) || max_drain_reached) break;

            if (!conn_state_->is_running(PROC_TRANSPORT)) {
                if (complete_responses < sent) keep_alive_working = false;
                break;
            }
        ...
        if (!accumulated_response.empty()) {
            printf("[WARN] Partial response remaining (%zu bytes)\n", accumulated_response.size());
        }
        ...
        printf(" bytes\n");
        printf("  Size range:        min=%zu, max=%zu, avg=%zu\n", min_size, max_size, avg_size);
        ...
        printf("  MSG_METADATA: prod=%ld, cons=%ld, caught_up=%s\n",
               metadata_prod_seq, metadata_cons_seq, metadata_caught_up ? "yes" : "NO");
        ...
        printf("  MSG_OUTBOX: prod=%ld, cons=%ld, caught_up=%s\n",
               outbox_prod_seq, outbox_cons_seq, outbox_caught_up ? "yes" : "NO");
        ...
        if (!keep_alive_working) {
            printf("\nFAIL: HTTP/1.1 keep-alive not working\n");
            return false;
        }
        ...
        if (complete_responses < sent) {
            printf("\nFAIL: Only %d/%d responses\n", complete_responses, sent);
            return false;
        }
        if (!metadata_caught_up || !outbox_caught_up) {
        ...
        if (!sizes_similar) {
            printf("\nFAIL: Response sizes differ too much\n");
            return false;
        }

        printf("\nPASS: HTTPS/1.1 keep-alive working (OpenSSL)\n");
        return true;
    }
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        ...
        fprintf(stderr, "NOTE: Use ./scripts/test_xdp.sh 13_transport_https_openssl.cpp\n");
        return 1;
    }
    ...
    printf("==============================================\n");
    printf("  Transport HTTPS Test (OpenSSLPolicy)        \n");
    printf("==============================================\n");
    printf("  Interface:  %s\n", interface);
    ...
    TransportHTTPSOpenSSLTest test(interface, bpf_path);
