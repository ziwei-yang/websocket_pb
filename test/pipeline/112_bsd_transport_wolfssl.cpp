// test/pipeline/112_bsd_transport_wolfssl.cpp
// Test BSDSocketTransportProcess with WolfSSL (both Linux and macOS)
//
// Usage:
//   ./test_pipeline_bsd_transport_wolfssl           # Test all (2-thread + 3-thread SSL)
//   ./test_pipeline_bsd_transport_wolfssl 2thread   # 2-thread InlineSSL only
//   ./test_pipeline_bsd_transport_wolfssl 3thread   # 3-thread DedicatedSSL only
//
// Tests:
//   - WolfSSLPolicy + BlockingIO + InlineSSL (2-thread) - nginx.org:443
//   - WolfSSLPolicy + BlockingIO + DedicatedSSL (3-thread) - nginx.org:443

#if !defined(HAVE_WOLFSSL) && !defined(WOLFSSL_USER_SETTINGS)
#error "This test requires WolfSSL (HAVE_WOLFSSL must be defined)"
#endif
...
public:
    IPCRingManager(const char* suffix = "") {
        ...
        ipc_ring_dir_ = std::string("bsd_wolfssl_test_") + timestamp + suffix;
    }
...
// ============================================================================
// Type Aliases for WolfSSL
// ============================================================================

using WolfSSL2Thread = BSDSocketTransportProcess<
    websocket::ssl::WolfSSLPolicy,
    DefaultBlockingIO,
    InlineSSL,
...
using WolfSSL3Thread = BSDSocketTransportProcess<
    websocket::ssl::WolfSSLPolicy,
    DefaultBlockingIO,
    DedicatedSSL,
    IPCRingConsumer<MsgOutboxEvent>,
...
private:
    ...
    bool fork_transport_process() {
        ...
        printf("[PARENT] Forked WolfSSL transport (PID %d)\n", transport_pid_);
    ...
    bool run_https_test() {
        ...
        snprintf(http_request, sizeof(http_request),
                 "GET / HTTP/1.1\r\n"
                 "Host: %s\r\n"
                 "User-Agent: WolfSSLTransportTest/1.0\r\n"
                 "Accept: */*\r\n"
                 "Connection: close\r\n"
                 "\r\n", host_);
...
int main(int argc, char* argv[]) {
    ...
    printf("================================================\n");
    printf("  WolfSSL Transport Test Suite\n");
    printf("================================================\n");
    ...
    if (test_2thread) {
        SSLTestRunner<WolfSSL2Thread> runner(
            "2-Thread WolfSSL (InlineSSL)",
            SSL_TEST_HOST, SSL_TEST_PORT, "_wolfssl2");
        if (runner.run()) passed++; else failed++;
        usleep(200000);
    }

    if (test_3thread) {
        SSLTestRunner<WolfSSL3Thread> runner(
            "3-Thread WolfSSL (DedicatedSSL)",
            SSL_TEST_HOST, SSL_TEST_PORT, "_wolfssl3");
        if (runner.run()) passed++; else failed++;
    }
