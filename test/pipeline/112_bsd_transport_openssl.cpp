// test/pipeline/112_bsd_transport_openssl.cpp
// Test BSDSocketTransportProcess with OpenSSL (both Linux and macOS)
//
// Usage:
//   ./test_pipeline_bsd_transport_openssl           # Test all (2-thread + 3-thread SSL)
//   ./test_pipeline_bsd_transport_openssl 2thread   # 2-thread InlineSSL only
//   ./test_pipeline_bsd_transport_openssl 3thread   # 3-thread DedicatedSSL only
//
// Tests:
//   - OpenSSLPolicy + BlockingIO + InlineSSL (2-thread) - nginx.org:443
//   - OpenSSLPolicy + BlockingIO + DedicatedSSL (3-thread) - nginx.org:443

#if defined(HAVE_WOLFSSL) || defined(WOLFSSL_USER_SETTINGS)
#error "This test requires OpenSSL, not WolfSSL"
#endif
...
public:
    IPCRingManager(const char* suffix = "") {
        ...
        ipc_ring_dir_ = std::string("bsd_openssl_test_") + timestamp + suffix;
    }
...
using OpenSSL2Thread = BSDSocketTransportProcess<
    websocket::ssl::OpenSSLPolicy,
    DefaultBlockingIO,
    InlineSSL,
...
using OpenSSL3Thread = BSDSocketTransportProcess<
    websocket::ssl::OpenSSLPolicy,
    DefaultBlockingIO,
    DedicatedSSL,
    IPCRingConsumer<MsgOutboxEvent>,
...
private:
    ...
    bool fork_transport_process() {
        ...
        printf("[PARENT] Forked OpenSSL transport (PID %d)\n", transport_pid_);
    ...
    bool run_https_test() {
        ...
        snprintf(http_request, sizeof(http_request),
                 "GET / HTTP/1.1\r\n"
                 "Host: %s\r\n"
                 "User-Agent: OpenSSLTransportTest/1.0\r\n"
                 "Accept: */*\r\n"
                 "Connection: close\r\n"
                 "\r\n", host_);
...
int main(int argc, char* argv[]) {
    ...
    printf("================================================\n");
    printf("  OpenSSL Transport Test Suite\n");
    printf("================================================\n");
    ...
    if (test_2thread) {
        SSLTestRunner<OpenSSL2Thread> runner(
            "2-Thread OpenSSL (InlineSSL)",
            SSL_TEST_HOST, SSL_TEST_PORT, "_openssl2");
        if (runner.run()) passed++; else failed++;
        usleep(200000);
    }

    if (test_3thread) {
        SSLTestRunner<OpenSSL3Thread> runner(
            "3-Thread OpenSSL (DedicatedSSL)",
            SSL_TEST_HOST, SSL_TEST_PORT, "_openssl3");
        if (runner.run()) passed++; else failed++;
    }
