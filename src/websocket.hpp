...
struct WebSocketClient {
    ...
    // Detect ShmRingBuffer (requires runtime path)
    template<typename T, typename = void>
    struct is_shm_ringbuffer_trait : std::false_type {};

    template<typename T>
    struct is_shm_ringbuffer_trait<T, std::void_t<decltype(T::is_shm_ringbuffer)>>
        : std::bool_constant<T::is_shm_ringbuffer> {};

    // Used only in constructor to check if path is required
    static constexpr bool requires_runtime_path = is_shm_ringbuffer_trait<RxBufferPolicy_>::value;
    ...
    // rx_shmem_path: if provided, attaches to shared memory and disables on_messages callback
    // if nullptr: allocates private memory and enables on_messages callback
    WebSocketClient(const char* rx_shmem_path = nullptr)
        : connected_(false)
        , xdp_initialized_(false)
        , rx_callback_enabled_(rx_shmem_path == nullptr)  // Disable callback if shmem path provided
        , msg_count_(0)
    {
        try {
            if constexpr (requires_runtime_path) {
                // ShmRingBuffer: requires path
                if (rx_shmem_path) {
                    rx_buffer_.init(rx_shmem_path);
                } else {
                    throw std::runtime_error("ShmRingBuffer requires shmem path");
                }
            } else {
                // RingBuffer or HftShmRingBuffer: init as usual
                rx_buffer_.init();
            }
    ...
    bool recv_into_buffer() {
        ...
        while (true) {
            ...
            if (n > 0) {
                ...
                data_written_ = write_offset + static_cast<size_t>(n);
...
private:
    ...
    size_t data_written_ = 0;             // Bytes of SSL data written in current batch (not yet committed)
