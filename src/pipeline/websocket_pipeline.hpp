...
//   pipeline.start();
//   // ... consume ws_frame_info or MktEventHandler handles inline ...
//   pipeline.shutdown();
//
...
struct DefaultPipelineConfig {
    ...
    // ── Feature toggles ──
    static constexpr size_t MAX_CONN       = 16;
    ...
    static constexpr size_t CONN_PER_IP = 1;  // connections per unique IP (2 for 2-URL split)
};
...
public:
    ...
    // Create a standalone ring at /dev/shm/hft/<name>.{hdr,dat} (not in pipeline subdirectory).
    static bool create_standalone_ring(const char* name, size_t buffer_size,
                                        size_t event_size, uint8_t max_consumers = 1) {
        std::string base_path = std::string("/dev/shm/hft/") + name;
        std::string hdr_path = base_path + ".hdr";
        std::string dat_path = base_path + ".dat";
...
public:
        ...
        // IP Probe: resolve all IPs, measure RTT, rank
        {
            ...
            if constexpr (MaxConn > 1) {
                ...
                for (size_t i = 0; i < MaxConn; ++i) {
                    ...
                    // else: stays 0 (dormant — never SYN'd)
                }
    ...
    // WSFrameInfo region — available when !HasMktEventHandler or WS_FRAME_INFO_RING=true
